...
function publish(team_id, syncAccounts, title, intro, opts,
    filepath, category, torrent_filename) {
  var queue = [];
  var epName = [];
  ...
  return function (callback) {
    ...
    ep.once('torrent_buf', function (tbuf) {
      epName.forEach(function (btsname, i) {
        ...
        if (torrent_filename) {
          bts.setTorrentFilename(torrent_filename);
        }
        var q = queue[i];
          ...
          bts.login(function (err, islogin) {
            ...
            if(!err && islogin) {
              ...
              var fn_start_upload = function (err, retries) {
                ...
                bts.upload(title, intro, opts, tbuf, function (err, succeed) {
                  if (err) {
                    //console.error(err);
...
function update(team_id, syncAccounts, syncStatus, title, intro,
    filepath, category, torrent_filename) {
  var queue = [];
  var epName = [];
  for (var i = 0; i < syncAccounts.length; i++) {
    if (syncAccounts[i].enable && syncAccounts[i].site && syncAccounts[i].username
      ...
      if (syncStatus && syncStatus[q.site]) {
        q.status = syncStatus[q.site];

        var url = syncStatus[q.site];
        ...
        bts.login(function (err, islogin) {
          ...
          if(!err && islogin) {
            ...
            var fn_start_upload = function (err, retries) {
              if (retries) {
                if (retries >= 3) {
                  ep.emit(btsname, (err ? err : 'update failed'));
                  return;
                }
              } else {
                retries = 0;
              ...
              if (!q.url) {
                // TODO: reupload
                ep.emit(btsname, q.status);
              } else {
                bts.update(q.url, title, intro, function (err, succeed) {
                  if (err) {
                    //console.error(err);
                    if ((typeof err == 'string' && err == 'unknown error')
                      || (typeof err == 'object' && err.code == 'ETIMEDOUT')) {
...
function *main(team, torrent, filepath, category_tag_id, torrent_filename) {
  ...
  var status = yield publish(team._id, accounts,
    torrent.title, content, { btskey: torrent.btskey },
    filepath, cateName, torrent_filename);
...
function TeamSync(team, torrent, filepath, category_tag_id, torrent_filename, cb) {
  ...
  setImmediate(function () {
    ...
    var fn = co.wrap(main);
    fn.call(ctx, team, torrent, filepath, category_tag_id, torrent_filename)
      .then(function (res) {
        if (cb) cb(null, res)
      })
      .catch(function (err) {
