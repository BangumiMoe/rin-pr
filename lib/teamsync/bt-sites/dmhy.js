...
BTSiteDmhy.prototype.IsLogin = function (callback) {
  request.clearCookie(DMHY_BASE_URL);
  if (!this.m_cookie) {
    callback(null, false);
  } else {
    //check login
    request.setCookie(this.m_cookie, DMHY_BASE_URL);
    request.get(DMHY_BASE_URL + '/topics/add', function (err, response, body) {
      if (err) {
        callback(err);
        return;
      }
...
BTSiteDmhy.prototype.GetVcode = function (mode, callback) {
  request.get(DMHY_BASE_URL + this.m_vcode_url, { buffer: true }, function (err, response, body) {
    if (err) {
      callback('验证码获取失败');
      return;
    }
...
BTSiteDmhy.prototype.LoginEx = function (callback) {
  var that = this;
  request.get(DMHY_BASE_URL + '/user/login?goto=%2Ftopics%2Fadd', function (err, response, body) {
    if (err) {
      callback(err);
      return;
    }
    ...
    if (m && m[1]) {
      ...
      that.GetVcode('login', function (err, word) {
        ...
        request.post(DMHY_BASE_URL + '/user/login', form, function (err, response, body) {
          if (err) {
            callback(err);
            return;
          }
...
BTSiteDmhy.prototype.LoginSucceed = function (callback) {
  var str_cookie = request.getCookie(DMHY_BASE_URL);
...
BTSiteDmhy.prototype.UploadEx = function (formdata, callback) {
  var that = this;
  request.post(DMHY_BASE_URL + '/topics/add',
...
BTSiteDmhy.prototype.getlastpublish = function (callback) {
  ...
  request.get(DMHY_BASE_URL + '/topics/mlist/scope/team', function (err, response, body) {
    if (err) {
      callback(err);
      return;
    }
