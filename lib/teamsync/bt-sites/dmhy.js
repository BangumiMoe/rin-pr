...
BTSiteDmhy.prototype.upload = function (title, intro, opts, torrent_buf, callback) {
  // no need for vcode

  // get poster URL
  var posterUrl = this.getPosterUrl(intro);

  var formdata = {
    bt_data_title: title,
    poster_url: posterUrl,
    bt_data_intro: intro,
    emule_resource: this.getEd2kUrl(intro),
    synckey: opts.btskey || '',
  };
...
BTSiteDmhy.prototype.update = function (url, title, intro, callback) {
  // get poster URL
  var posterUrl = '';
  ...
  var formdata = {
    sort_id: this.m_options.sort_id,
    bt_data_title: title,
    poster_url: posterUrl,
    bt_data_intro: intro,
    submit: this.m_options.submit,
    bt_data_id: tid,
  };

  var that = this;
  this.request.post(DMHY_BASE_URL + '/topics/edit/id/' + tid,
    formdata, { multipart: true },
    function (err, response, body) {
      if (err) {
        callback(err);
