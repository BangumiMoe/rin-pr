...
function BTSiteAcgnx(opts) {
  ...
  this.setSite('acgnx')
  this.acgnxUrl = 'https://share.acgnx.se'
  this.m_options = {
    'mod': 'upload',
...
BTSiteAcgnx.prototype.IsLogin = function (callback) {
  this.request.clearCookie(this.acgnxUrl)
  // api token
  if (this.m_password) {
    callback(null, true)
  } else {
...
BTSiteAcgnx.prototype.upload = function (title, intro, opts, torrent_buf, callback) {
  //no need for vcode
  ...
  var that = this;
  this.request.post(this.acgnxUrl + '/user.php?o=api&op=upload',
      ...
      if (body.status === 'success') {
        if (body.infohash) {
          that.m_lastone = {
            ...
            url: that.acgnxUrl + '/show-' + body.infohash + '.html',
          }
        }
        callback(null, true)
      } else {
