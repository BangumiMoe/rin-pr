...
var NYAA_BASE_URL = 'https://www.nyaa.se';

function nyaaUrlPrefix(url, https) {
  if (https && url.indexOf('http:') >= 0) {
    return url.replace('http:', 'https:');
  } else if (!https && url.indexOf('https:') >= 0) {
    return url.replace('https:', 'http:');
  } else if (url[0] === '/' && url[1] === '/') {
    return (https ? 'https:' : 'http:') + url;
  } else if (url[0] === '/') {
    url = NYAA_BASE_URL + url;
    if (!https) {
...
BTSiteNyaa.prototype.getUserId = function (cb) {
  if (this.user_id) {
    return cb(null, this.user_id)
  }
  const that = this
  this.request.get(NYAA_BASE_URL + '/?page=profile', function (err, response, body) {
    ...
    if (response.statusCode === 200 && body) {
      ...
      if (iboxpos !== -1) {
        ...
        if (m) {
          var m2 = m[1].match(/\/?user=(\d+)/);
          if (m2) {
            const user_id = parseInt(m2[1])
...
BTSiteNyaa.prototype.getlastpublish = function (callback) {
  ...
  var cb = function (err, lo) {
    if (lo && lo.url) {
      ...
      lo.url = nyaaUrlPrefix(lo.url, false);
    }
    callback(err, lo);
  };
  ...
  this.getUserId((err, user_id) => {
    var torrentsUrl = nyaaUrlPrefix('/?user=' + user_id);
...
BTSiteNyaa.prototype.listmytorrents = function (p, callback) {
  ...
  const next = (user_id) => {
    ...
    that.request.get(nyaaUrlPrefix(url), function (err, response, body) {
      if (err) {
        return callback(err)
      }
      if (response.statusCode === 200 && body) {
        const m1 = body.match(/<table class="tlist">(.*?)<\/table>/i)
        if (m1) {
          body = m1[1]
          const m_divs = body.match(/<td class="tlistname">.*?<a href="(.*?)">(.*?)<\/a>/ig)
          const torrents = m_divs.map((div) => {
            const m = div.match(/<a href="(.*?)">(.*?)<\/a>/i)
            if (m) {
              const url = S(m[1]).decodeHTMLEntities().s
              const title = S(m[2]).decodeHTMLEntities().s
              const m_tid = url.match(/&tid=(\d+)/i)
              let tid
              if (m_tid) tid = parseInt(m_tid[1])
              return { tid, url, title }
            }
            return null
          })
...
BTSiteNyaa.prototype.remove = function (url, callback) {
  const m_tid = url.match(/&tid=(\d+)/i)
  if (m_tid) {
    const tid = parseInt(m_tid[1])
    if (tid) {
      const formdata = {
        [`tid-${tid}`]: tid,
        redirect: '//www.nyaa.se/?page=view&tid=' + tid,
        tid,
        mod: 1,
        move: '1_38',
        submit: 'Submit',
      ...
      const that = this
      this.request.post(nyaaUrlPrefix('/?page=manage&op=1'), formdata, function (err, response, body) {
        if (err) {
          return callback(err)
        }
        if (response.statusCode === 200 && body) {
          let iboxpos = body.indexOf('<div class="content">')
          if (iboxpos !== -1) {
            iboxpos += 21
            const iboxposend = body.indexOf('</div>', iboxpos)
            body = body.substr(iboxpos, iboxposend - iboxpos)
            if (body) {
              if (/Torrent\(s\) deleted!/.test(body)) {
                callback(null, true)
              } else {
                callback(body)
              }
