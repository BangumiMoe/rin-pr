...
var calcSimilarityByFiles = function (files1, files2) {
  ...
  function get_new_files_array(files) {
    ...
    if (nf > 3) {
      // only keep the first 3 larger files
      nf = _.sortBy(nf, function (r) { return -r[1]; });
      nf = nf.slice(0, 3);
    }
...
var predictTitle = function (orig_title, orig_files, new_files, ret) {
  if (!orig_title) {
    return '';
  }
