...
Torrents.prototype.getSuggestByFiles = function *(files, user_id, team_id) {
    ...
    if (rs) {
        ...
        rs.forEach(function (t) {
            t.similarity = calcSimilarityByFiles(files, t.content);
            if (t.similarity > 0.7) {
        ...
        if (rs.length) {
          ...
          var torrents = yield this.collection.find({ _id: { $in: ids } },
              { title: 1, introduction: 1, category_tag_id: 1, tag_ids: 1, team_id: 1, teamsync: 1, btskey: 1 })
              .toArray();
          var rtorrents = [];
          for (var i = 0; i < rs.length; i++) {
...
var calcSimilarityByFiles = function (files1, files2) {
  ...
  if (nf1.length === 1 && nf2.length === 1) {
    return calcSimilarityByTitleLevenshtein(nf1[0][0], nf2[0][0]);
    ...
    if (nf1.length === 1 || nf2.length === 1) {
      ...
      for (var i = 0; i < nf2.length; i++) {
        totalsize += nf2[i][1];
        s += nf2[i][1] * calcSimilarityByTitleLevenshtein(nf1[0][0], nf2[i][0]);
      }
      s /= totalsize;
      return s;
    } else {
      ...
      for (var i = 0; i < nf1.length; i++) {
        for (var j = 0; j < nf2.length; j++) {
          s += (nf1[i][1] * nf2[j][1] / totalsize)
            * calcSimilarityByTitleLevenshtein(nf1[i][0], nf2[j][0]);
        }
      }
      return s;
    }
